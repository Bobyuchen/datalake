...
from .assets import my_dbt_trino_project_dbt_assets


@job
def Transform_rowdata_job():
    import os
    import json
    import pandas as pd
    import trino
...
# 使用 build_schedule_from_dbt_selection 創建排程
schedules = [
    build_schedule_from_dbt_selection(
        [my_dbt_trino_project_dbt_assets],
        job_name="materialize_every_hour",
        cron_schedule="10 * * * *",  # 每小時整點運行
        dbt_select="fqn:*base*",  # 選擇名稱中包含 'base' 的資產
    ),
    build_schedule_from_dbt_selection(
        [my_dbt_trino_project_dbt_assets],
        job_name="materialize_omop_every_hour_20_min",
        cron_schedule="20 * * * *",  # 每小時第 20 分鐘運行一次
        dbt_select="fqn:*omop*",  # 選擇名稱中包含 'omop' 的資產
    ),
    ScheduleDefinition(
        job=Transform_rowdata_job,
        cron_schedule="0 * * * *",  # 每小時第 50 分鐘運行一次
    ),
]
